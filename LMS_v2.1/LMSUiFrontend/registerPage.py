...
class Register(QFrame):
    ...
    def registerWidgets(self):
        ...
        self.classStartMonth = self.findChild(QComboBox, "classStartCombo")
    ...
    def submitEntry(self):  # returns a list of tuples (school entries)
        self.register_list = []
        ...
        if self.department.currentIndex() != 0:
            ...
            class_Entries = [self.department.currentText(), sy, self.classStartMonth.currentText(),
                             self.school_head.text().upper(), self.grade.currentText(),
                             self.section.text().upper(), self.adviser.text().upper()]
            # Checking if entries are not blank and remove whitespace
            if all(class_Entry.strip() for class_Entry in class_Entries):
                ...
                if self.department.currentIndex() == 1:
                    ...
                    class_valid_entries.extend([self.strand.currentText()])
